...
const GEMINI_MODEL = "gemini-2.0-flash"; // Switched to 2.0-flash because 2.5-flash is throwing 503 overloaded errors
...
const safeGenerateJson = async ({ prompt, fallback }) => {
  ...
  try {
    ...
    const result = await withTimeout(model.generateContent(prompt), 12000, "Gemini request timed out");
